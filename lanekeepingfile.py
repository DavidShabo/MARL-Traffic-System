...
class RLLibMetaDriveRoundabout(MultiAgentEnv):
    ...
    def _custom_reward(self, obs, info):
        ...
        for agent_id in obs.keys():
            ...
            try:
                ...
                ### LANE KEEPING (Multiplier Style, RF1)
                if vehicle.lane in vehicle.navigation.current_ref_lanes:
                    ...
                    positive_road = 1.0 if not current_road.is_negative_road() else -1.0

                long_last, _ = current_lane.local_coordinates(vehicle.last_position)
                long_now, lateral_now = current_lane.local_coordinates(vehicle.position)

                # Lane factor: boosts progress reward when centered (0=edges, 1=center)
                lane_width = vehicle.navigation.get_current_lane_width()
                lateral_factor = np.clip(1 - 2 * abs(lateral_now) / lane_width, 0.0, 1.0)

                # Apply to progress (preferred) OR as standalone bonus
                reward += 10.0 * (long_now - long_last) * lateral_factor * positive_road  #


                ### DESTINATION ARRIVAL - Bonus for completing the route
                if agent_info.get("arrive_dest", False):
                    reward += 20.0

                ### SPEED & STEERING CONTROL - Slow in real turns, penalize zigzag
                ...
                ### SAFETY PENALTIES - Don't crash into objects or agents
                ...
                ### COOPERATIVE BEHAVIOR - Penalty for being too close to other agents
    ...
    def step(self, action_dict):
        ...
        self._agent_ids = set(self.env.agents.keys()) | set(obs.keys())
...
def build_env_config(num_agents: int, render: bool) -> dict:
    return {
        ...
        "traffic_density": 0.3,      # No background traffic during early training (reduce noise)
        "allow_respawn": True,  # Already good
        "delay_done": 25,       # Add: Hold dead agents longer
        "vehicle_config": {
